...
public partial class UserControl_CongViec : System.Windows.Forms.UserControl
{
    ...
    private void LoadData(IQueryable<Role> query = null)
    {
        try
        {
            if (query == null) query = context.Roles.AsQueryable();
            ...
            if (dgvCongViec.Columns.Count > 0)
            {
                ...
                if (dgvCongViec.Columns.Contains("Users")) dgvCongViec.Columns["Users"].Visible = false;
            }
        }
        catch (Exception ex)
        {
    ...
    private void btnXoa_Click(object sender, EventArgs e)
    {
        ...
        {
            MessageBox.Show("Vui lòng chọn một công việc để xoá.", "Chưa chọn", MessageBoxButtons.OK,
        ...
        {
            MessageBox.Show($"Không thể xoá vai trò hệ thống '{roleName}'!", "Lỗi", MessageBoxButtons.OK,
                MessageBoxIcon.Error);
            return;
        }

        if (MessageBox.Show($"Bạn có chắc chắn muốn xoá công việc '{roleName}' không?", "Xác nhận xoá",
            ...
            {
                MessageBox.Show("Không thể xoá vai trò này. Có thể nó đang được gán cho một nhân viên.", "Lỗi",
